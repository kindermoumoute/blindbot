...
func (b *BlindBot) validateAnswer(ev *slack.MessageEvent) error {
	...
	if exist {
		...
		if matchAnswers(ev.Text, entry.answers) {
			...
			err := b.client.AddReaction("clap", slack.NewRefToMessage(ev.Channel, ev.Timestamp))
			if err != nil {
				return err
			}
			return b.client.AddReaction("heavy_check_mark", slack.NewRefToMessage(ev.Channel, ev.ThreadTimestamp))
		}
		return b.client.AddReaction("x", slack.NewRefToMessage(ev.Channel, ev.Timestamp))
	}
	return nil
}
